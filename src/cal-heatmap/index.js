      ...
      .call(function (selection) {
        ...
        if (options.tooltip) {
          selection.on('mouseover', function (d) {
            ...
            self.tooltip.html(self.getSubDomainTitle(d)).attr('style', 'display: block;');

            var tooltipPositionX =
              self.positionSubDomainX(d.t) - self.tooltip[0][0].offsetWidth / 2 + options.cellSize / 2;
...
CalHeatMap.prototype = {
  ...
  init: function (settings) {
    ...
    if (d3.select(document).select(options.itemSelector)[0][0] === null) {
    ...
    function validateSelector(selector, canBeFalse, name) {
      if (((canBeFalse && selector === false) || typeof selector === 'string') && selector !== '') {
        return true;
      }
    ...
    rect
      ...
      .attr('class', function (d) {
        ...
        if (
          ...
          htmlClass.push('graph-rect'); //graph-rect
        }
